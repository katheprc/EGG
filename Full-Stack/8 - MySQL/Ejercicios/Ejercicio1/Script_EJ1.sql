...
select nombre as Nombre, (sal_emp + comision_emp + 500) as ValorAPagar from empleados where id_depto = 3000;
...
select sal_emp, comision_emp, nombre, (sal_emp + comision_emp) as SalarioTotal from empleados where comision_emp > 1000;
...
select nombre, comision_emp, sal_emp from empleados where comision_emp <= (sal_emp*30/100);
...
select * from departamentos  where nombre_depto like "ventas" or nombre_depto like "investigación" or nombre_depto like "mantenimiento";



/* 20. Ahora obtener el contrario, los nombres de los departamentos que no sean “Ventas” ni “Investigación” ni ‘Mantenimiento. */

select * from departamentos  where nombre_depto not like "ventas" and nombre_depto not like "investigación" and nombre_depto not like "mantenimiento";
...
select avg(sal_emp) as SalarioPromedio from empleados;
