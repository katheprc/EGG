...
import Ejercicio1.Service.ServicePersona;
	...
	public static void main(String[] args) {
		...
		for (int aux = 1 ; aux < 6; aux++)  {
			...
			switch (aux) {
				...
				case 5: 
					try {
						srvPersona.crearPersona(persona5);
						if (persona5.getEsMayor() == true) {
							auxMayor += 1;
						}
						if (persona5.getResultadoImc() == -1) {
							auxBajo +=1;
						} else if (persona5.getResultadoImc() == 0) {
							auxIdeal +=1;
						} else {
							auxSobre +=1;
						}
					}catch(Exception e) {
						System.out.println(e.getMessage()); //Cannot invoke "Ejercicio1.Entity.Persona.setNombre(String)" because "persona" is null
					}
					break;
